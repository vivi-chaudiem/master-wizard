...
def run_roles_chain(llm, product, production_steps, json_template):
    ...
    template = "Angenommen, in der Produktion des Produkts {product} sind folgende Arbeitsschritte definiert: {production_steps}. Welche Rollen könnten diesen Arbeitsschritten zugeordnet werden? (Beispiel: Dem Arbeitsschritt 'Montage' könnte die Rolle 'Monteur' zugeordnet sein.) Anbei eine Liste mit Rollen und den dazugehörigen Arbeitsschritten, die bereits bekannt sind: {existing_roles}. Beachte, dass die Antwort nicht auf die vorgegebene Liste beschränkt ist und auch darüber hinausgehende Möglichkeiten berücksichtigt werden sollen. Wenn es semantische Überschneidungen bei den Rollen zwischen deiner Antwort und der Liste gibt, passe deine Formulierung entsprechend an, um Doppelungen zu vermeiden. Arbeitsschritte können mehrmals vorkommen, sofern es hierfür mehrere Rollen gibt. Weder Arbeitsschritt noch Rolle dürfen leer sein. Bitte gib mir deine Antwort direkt als JSON zurück (ohne einleitende Worte) und strukturiere die Antwort gemäß der folgenden Vorlage:\n"
