...
# Roles chain
def run_roles_chain(llm, product, production_steps, json_template, additionalCompanyInfo, additionalProductInfo):
    ...
    prompt = PromptTemplate(
        input_variables=["product", "production_steps", "additionalProductInfo", "additionalCompanyInfo", "existing_roles"], template=formatted_template
        )
    
    chain = LLMChain(llm=llm, prompt=prompt)
    print(formatted_template)

    return chain.run({"product": product, "production_steps": production_steps, "additionalProductInfo": additionalProductInfo, "additionalCompanyInfo": additionalCompanyInfo, "existing_roles": existing_roles})
