...
def run_production_steps_chain(llm, product, additionalCompanyInfo, additionalProductInfo):
    ...
    template = "Bitte erstelle eine detaillierte Liste von Arbeitsschritten, die spezifisch für die Produktion des Produkts '{product}' erforderlich sind. Falls zusätzliche Informationen zu Produkt und Unternehmen zur Verfügung stehen, berücksichtige diese zur Verfeinerung deiner Liste. Das sind die optional verfügbaren, zusätzlichen Produktinformationen: '{additionalProductInfo}' \nDas sind die optional verfügbaren, zusätzlichen Unternehmensinformationen: '{additionalCompanyInfo}'\nFalls diese Informationen fehlen sollten, arbeite nur mit dem Produktnamen '{product}'. Deine Aufgabe ist es, eine Liste von Arbeitsschritten zu entwickeln, die direkt mit der Herstellung des genannten Produkts verbunden sind. Anschließend sollst du diese Liste mit den bereits vorhandenen, generellen Arbeitsschritten auf semantische Überschneidungen vergleichen und deine Formulierung bei Bedarf anpassen, um Wiederholungen zu vermeiden, die folgendermaßen lauten: {existing_prod_steps}. Falls die Liste mit generellen Arbeitsschritten leer ist, überspringe diesen Schritt. Deine Antwort besteht nur aus der endgültigen Liste, die in eine nummerierte Form (z.B. '1.', '2.', '3.', etc.) hat und keine einleitenden Worte, zusätzliche Erklärungen oder Schlusssätze enthält. Deine Antwort muss also direkt mit 1. beginnen und mit dem letzten nummerierten Punkt aufhören.\nWichtig ist, dass die finale Liste der Arbeitsschritte direkt aus deiner ersten Analyse des Produktnamens, und falls vorhanden, aus den spezifischen Produkt- und Unternehmensinformationen abgeleitet wird. Sie soll nicht auf die in der übergebenen Liste enthaltenen Schritte beschränkt sein, aber dennoch semantische Überschneidungen mit dieser generellen Liste vermeiden. In jedem Fall sollte am Ende eine numerierte Liste als Antwort gegeben werden."
...
def run_roles_chain(llm, product, production_steps, json_template, additionalCompanyInfo, additionalProductInfo, additionalRolesInfo):
    ...
    template = "Angenommen, in der Produktion des Produkts '{product}' sind folgende Arbeitsschritte definiert: {production_steps}. Außerdem gibt es folgende zusätzliche Informationen:\nZum Produkt: {additionalProductInfo} \nZum Unternehmen: {additionalCompanyInfo}. Welche Rollen es für jeden dieser Arbeitsschritte geben? (Beispiel: Dem Arbeitsschritt 'Montage' könnte die Rolle 'Monteur' zugeordnet sein.) Anbei zusätzliche Informationen zu den Rollen: {additionalRolesInfo}. Außerdem anbei eine Liste mit Rollen und den dazugehörigen Arbeitsschritten, die bereits bekannt sind: {existing_roles}. Beachte, dass die Antwort nicht auf die vorgegebene Liste beschränkt ist und auch darüber hinausgehende Möglichkeiten berücksichtigt werden sollen. Wenn es semantische Überschneidungen bei den Rollen zwischen deiner Antwort und der Liste gibt, passe deine Formulierung entsprechend an, um Doppelungen zu vermeiden. Arbeitsschritte können mehrmals vorkommen, sofern es hierfür mehrere Rollen gibt. Weder Arbeitsschritt noch Rolle dürfen leere Werte beinhalten. Bitte gib mir deine Antwort direkt als JSON zurück (ohne einleitende Worte) und strukturiere die Antwort gemäß der folgenden Vorlage:\n"
